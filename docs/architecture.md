...
Two deployment modes share one adapter. The default is authenticated, single-user, and stdio, and reads through `BrowserSession`. `--public-only` restricts `ReadOnlyGuard` to the public Waseda hosts, skips the stored storage state, the academic profile, and the mapping cache, and registers only the catalog tools.

The public sources need no browser. Web Syllabus detail pages are ordinary `GET` responses and its search is a stateless form `POST` that works without a session cookie, so `FetchPageReader` implements the same `SyllabusPageReader` interface over plain HTTP and every parser, guard, and limiter is reused unchanged. It posts url-encoded fields rather than the form's declared multipart so the guard inspects exactly the bytes that are sent.

The HTTP surface is one web-standard `fetch` app. A Node server mounts it through `toNodeHandler`, and a host that captures a `server` entrypoint runs the same app, so `/mcp`, the JSON API, and the static web UI route identically everywhere. Client fairness lives in that layer; upstream volume stays bounded by the serial limiter and the normalized cache, both of which are per-process and therefore weaker on a host that runs several instances.
