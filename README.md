...
## ネイティブ・コンパニオン（macOS / iOS / Android）

`clients/waseda_portal` は、公開Webとは別の端末内コンパニオンです。各端末のアプリ内WebViewで利用者本人がMyWaseda／Waseda Moodleへログインし、ログイン後に所属・在学年次を自動検出します。大学のID・パスワード、Cookie、氏名、学生番号、成績をこのリポジトリのサーバーやVercelへ送ることはありません。利用者がシラバス検索を実行した場合だけ、検索条件として科目名・所属・年次を公開検索Webへ渡します。

```bash
cd clients/waseda_portal
flutter pub get
flutter run -d macos
```

Android／iOSでも同じFlutterコードを使用します。Moodle／MyWaseda上の画面遷移やフォーム送信はアプリ側で制限せず、公式サイトと同様に操作できます。「この画面を読み取る」は、ログイン後に画面内の予定・科目名を端末内の一覧へ表示する補助機能で、結果はアプリ終了までのメモリにだけ保持します。これは初期実装であり、実機でのMoodleログイン・ページ構造・通常操作の確認後に配布可能と判断します。
